...
# 결측치 표현 & 처리  
data2 = np.array([1, 2.5,  np.nan, 3.3, 4.6, np.nan])

np.isnan(data2) # 결측치(True) 

# 결측치 제외 : 조건식 이용 
data2[~ np.isnan(data2)] # [1. , 2.5, 3.3, 4.6]

data2[np.isnan(data2)] # [nan, nan]
...
# 3. axis 속성 
print(data2.sum(axis=0)) # 행 축  
print(data2.sum(axis=1)) # 열 축 
print('전체 원소 합계 :', data2.sum()) # 축 생략 
